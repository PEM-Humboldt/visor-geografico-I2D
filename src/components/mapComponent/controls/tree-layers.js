...
export function buildLayerTree(layer) {
  ...
  for (var i = 0; i < layers.length - numcap; i++) {
    ...
    if (!group || typeof group.getLayers !== "function") {
      console.warn(
        "buildLayerTree: Skipping invalid group (likely VectorLayer):",
        group.get("title") || group.get("name") || "unnamed",
      );
      continue;
    }
    ...
    for (var j = 0; j < leng; j++) {
      ...
      check.onclick = function (ev) {
        ...
        if (layerToToggle && typeof layerToToggle.setVisible === "function") {
          ...
          console.log(
            `Layer ${layerToToggle.get("title")} visibility set to: ${ev.target.checked}`,
          );
        }
      };
      ...
      if (lay[j].values_.urldownload && lay[j].values_.urldownload != "") {
        ...
        down.setAttribute(
          "onclick",
          'window.open("' + lay[j].values_.urldownload + '")',
        );
...
function createCombinedCardsForGroups(
  groupNames,
  combinedCards,
  parentElement,
  groups,
  ...
  for (var i = 0; i < groupNames.length; i++) {
    ...
    combinedCollapse.className = "collapse";
    combinedCollapse.setAttribute(
      "aria-labelledby",
      "#combinedCollapse" + i + aa,
    );
