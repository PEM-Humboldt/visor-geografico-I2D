...
//capa para seleccion
export var highlight = new VectorLayer({
  style: styleHighlight,
  source: new VectorSource(),
});
export var highlightPoint = new VectorLayer({
  style: styleHighlightPoint,
  source: new VectorSource(),
});
...
const createDynamicWMSLayer = (layerConfig) => {
  ...
  if (layer && typeof layer.setVisible === "function") {
    ...
    if (actualVisibility !== Boolean(layerConfig.estado_inicial)) {
      console.warn(
        `Layer ${layerConfig.nombre_display}: visibility mismatch! Expected: ${layerConfig.estado_inicial}, Actual: ${actualVisibility}`,
      );
      // Force set again
      setTimeout(
        () => layer.setVisible(Boolean(layerConfig.estado_inicial)),
...
const createDynamicLayerGroups = async (project) => {
  ...
  for (const group of project.layer_groups) {
    ...
    if (groupLayers.length > 0) {
      const groupName =
        group.nombre_display || group.nombre || `Group ${group.id}`;
...
export const initializeLegacyExports = async () => {
  try {
    ...
    Object.keys(layers).forEach((key) => {
      ...
      if (key.startsWith("proyecto_oleoducto"))
        proyecto_oleoducto_bicentenario = layers[key];
      if (key.startsWith("conservacion"))
        conservacion_biodiversidad = layers[key];
...
export const hightlightAdd = (feature, type) => {
  type == "point"
    ? highlightPoint.getSource().addFeature(feature)
...
export const highlightStadisticsAdd = (feature) => {
  highlightStadistics.getSource().addFeature(feature);
};
// remove the feat selection
export const highlightStadisticsRemove = () => {
  highlightStadistics.getSource().clear();
};
